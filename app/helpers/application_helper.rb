module ApplicationHelper
  ...
  def show_attachment attachment
    case attachment.content_type
      when 'image/jpeg'
        link_to(image_tag(attachment.file.thumb.url, class: 'img-fluid'), attachment.file.url, target: '_blank', data: {lightbox: 'attached-img'})
      when 'image/gif'
        link_to(image_tag(attachment.file.thumb.url, class: 'img-fluid'), attachment.file.url, target: '_blank', data: {lightbox: 'attached-img'})
      when 'image/png'
        link_to(image_tag(attachment.file.thumb.url, class: 'img-fluid'), attachment.file.url, target: '_blank', data: {lightbox: 'attached-img'})
      when 'video/quicktime'
        ...
        link_to "<i class=\"fa fa-file-video-o\"></i> #{attachment.file_identifier}".html_safe, attachment.file.url, target: '_blank', data: {lightbox: 'attached-video'}
      when 'video/mp4'
        video_tag attachment.file.url, class: 'img-fluid', controls: 'true', type: 'video/mp4', data: {lightbox: 'attached-video'}
